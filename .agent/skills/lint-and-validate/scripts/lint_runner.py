...
def get_cmd(cmd: str) -> str:
    """Get the correct command for the OS."""
    if sys.platform == "win32" and cmd in ["npm", "npx", "yarn", "pnpm"]:
        return f"{cmd}.cmd"
    return cmd
...
def detect_project_type(project_path: Path) -> dict:
    ...
    if package_json.exists():
        ...
        try:
            ...
            if "lint" in scripts:
                result["linters"].append({"name": "npm lint", "cmd": [get_cmd("npm"), "run", "lint"]})
            elif "eslint" in deps:
                result["linters"].append({"name": "eslint", "cmd": [get_cmd("npx"), "eslint", "."]})
            
            # Check for TypeScript
            if "typescript" in deps or (project_path / "tsconfig.json").exists():
                result["linters"].append({"name": "tsc", "cmd": [get_cmd("npx"), "tsc", "--noEmit"]})
